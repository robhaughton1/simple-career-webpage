# Career Information Webpage

This project is a simple HTML and CSS website created as part of an early web development assignment. It presents basic career information.

## Features
- Structured webpage built with semantic HTML
- External CSS stylesheet for layout and design
- Organized image folder for assets
- Simple, readable design suitable for beginners

## What I practiced
- Building a webpage from scratch using HTML
- Applying styles through an external CSS file
- Organizing project files (HTML, CSS, images)
- Creating a themed webpage with text and visuals

## Technologies Used
- HTML5
- CSS3

## How to View
Open `index.html` in any web browser to view the webpage.
